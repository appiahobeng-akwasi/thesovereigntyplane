...
import type { Country, View, Scope } from '../data/types';
...
export default function PlaneIsland({ countries }: Props) {
  ...
  const scopeCount = scope === 'africa'
    ? countries.filter((c) => c.in_africa_scope).length
    : countries.filter((c) => c.in_frontier_scope).length;
